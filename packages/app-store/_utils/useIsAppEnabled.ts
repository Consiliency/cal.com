...
function useIsAppEnabled(app: EventTypeAppCardApp, teamId?: number | null) {
  ...
  const updateEnabled = (newValue: boolean) => {
    ...
    if (newValue) {
      // Check if this is a manually configured app (has keys but no credentials)
      const isManuallyConfigured =
        app.keys &&
        !app.isInstalled &&
        !app.credentialOwner &&
        (!app.userCredentialIds || app.userCredentialIds.length === 0);

      if (isManuallyConfigured) {
        // Don't set credentialId for manually configured apps
        // The API will use the app keys instead
        console.log(`App ${app.slug} is manually configured, not setting credentialId`);
      } else {
